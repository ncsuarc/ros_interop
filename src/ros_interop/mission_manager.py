...
class MissionManager():
    ...
    def get_mission(self,req):
        ...
        mission_info.lost_comms_pos = position(GeoPoint(latitude = mission_data['lostCommsPos']['latitude'],longitude = mission_data['lostCommsPos']['longitude']))
        ...
        for fly_zone in mission_data['flyZones']:
            ...
            for boundary_point in fly_zone['boundaryPoints']:
                boundaryPoint = position(GeoPoint(latitude = boundary_point['latitude'], longitude = boundary_point['longitude']))
        ...
        for waypoint in mission_data['waypoints']:
            waypointObj = position(GeoPoint(waypoint['latitude'],waypoint['longitude'],waypoint['altitude']))
        ...
        for search_grid_point in mission_data['searchGridPoints']:
            searchGridPoint = position(GeoPoint(latitude = search_grid_point['latitude'],longitude = search_grid_point['longitude']))
            mission_info.search_grid_points.append(searchGridPoint)
        mission_info.off_axis_odlc_pos = position(GeoPoint(latitude = mission_data['offAxisOdlcPos']['latitude'],longitude =mission_data['offAxisOdlcPos']['longitude']))
        mission_info.emergent_last_known_pos = position(GeoPoint(latitude = mission_data['emergentLastKnownPos']['latitude'], longitude = mission_data['emergentLastKnownPos']['longitude']))
        mission_info.air_drop_boundary_points = []
        for air_drop_boundary_point in mission_data['airDropBoundaryPoints']:
            airDropBoundaryPoint = position(GeoPoint(latitude = air_drop_boundary_point['latitude'],longitude = air_drop_boundary_point['longitude']))
            mission_info.air_drop_boundary_points.append(airDropBoundaryPoint)
        mission_info.air_drop_pos = position(GeoPoint(latitude = mission_data['airDropPos']['latitude'],longitude = mission_data['airDropPos']['longitude']))
        mission_info.ugv_drive_pos = position(GeoPoint(latitude = mission_data['ugvDrivePos']['latitude'], longitude = mission_data['ugvDrivePos']['longitude']))
